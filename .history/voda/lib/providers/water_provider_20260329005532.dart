...
class WaterProvider extends ChangeNotifier {
  ...
  void _scheduleMidnightReset() {
    ...
    _midnightTimer = Timer(untilMidnight, () async {
      ...
      // New day — restore reminders regardless of yesterday's goal state.
  ...
  Future<void> _load() async {
    ...
    // Restore reminders after app restart — only if goal not already reached.
    if (_remindersEnabled && _currentIntake < _dailyGoal) {
      NotificationService.scheduleReminders(
        intervalMinutes: _reminderIntervalMinutes,
        startHour: _reminderStartHour,
        endHour: _reminderEndHour,
  ...
  Future<void> addWater(int ml) async {
    ...
    _log.add(ml);
    await _save();
    // Cancel today's remaining reminders the moment the goal is reached.
    if (!wasComplete && _currentIntake >= _dailyGoal && _remindersEnabled) {
  ...
  Future<void> removeLast() async {
    ...
    await _save();
    // Re-enable reminders if the user dips back below the goal.
    if (wasComplete && _currentIntake < _dailyGoal && _remindersEnabled) {
      NotificationService.scheduleReminders(
        intervalMinutes: _reminderIntervalMinutes,
        startHour: _reminderStartHour,
