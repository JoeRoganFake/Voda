...
class ClimateProvider extends ChangeNotifier {
  ...
  void attachWaterProvider(WaterProvider wp) {
    ...
    debugPrint('[Climate] attachWaterProvider: wiring up, autoClimate=${wp.autoClimateAdjust}');
  ...
  void _onWaterChanged() {
    ...
    if (!wp.autoClimateAdjust) {
      debugPrint('[Climate] _onWaterChanged: auto-adjust off, skipping');
      return;
    }

    final isNewDay = _lastAppliedDate != _today();
    debugPrint('[Climate] _onWaterChanged: autoJustTurnedOn=$autoJustTurnedOn, isNewDay=$isNewDay, status=$_status, lastApplied=$_lastAppliedDate');

    if (autoJustTurnedOn || isNewDay) {
      if (_status == ClimateStatus.loaded && _data != null) {
        _lastAppliedDate = _today();
        debugPrint('[Climate] applying cached adjustment: ${_data!.waterAdjustmentMl} ml');
        wp.applyClimateAdjustment(_data!.waterAdjustmentMl);
      }
      debugPrint('[Climate] calling refresh(force: $isNewDay)');
  ...
  /// Called by HomeScreen on mount. Always fetches fresh weather, applies the
  /// adjustment, and announces with a popup. Waits for WaterProvider prefs to
  /// be ready before checking autoClimateAdjust.
  Future<void> startupRefresh() async {
    // Wait a short beat so WaterProvider._load() finishes reading prefs.
    await Future.delayed(const Duration(milliseconds: 500));
    final wp = _waterProvider;
    if (wp == null || !wp.autoClimateAdjust) {
      debugPrint('[Climate] startupRefresh: auto-adjust off or no waterProvider, skipping');
      return;
    }
    debugPrint('[Climate] startupRefresh: fetching fresh weather...');
  ...
  Future<void> refresh({bool force = false}) async {
    debugPrint('[Climate] refresh(force=$force) cacheValid=${_isCacheValid()}');
    ...
    debugPrint('[Climate] fetching location...');
    ...
    try {
      ...
      _lastAppliedDate = _today();
      debugPrint('[Climate] fetch done: ${_data!.temperature}°C, ${_data!.humidity}%, adj=${_data!.waterAdjustmentMl} ml');
