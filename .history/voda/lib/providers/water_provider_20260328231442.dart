...
class WaterProvider extends ChangeNotifier {
  ...
  Future<void> applyClimateAdjustment(int adjustmentMl, {bool announce = false}) async {
    debugPrint('[Water] applyClimateAdjustment($adjustmentMl, announce=$announce) auto=$_autoClimateAdjust base=$_baseGoal current=$_dailyGoal');
    ...
    if (adjusted != _dailyGoal) {
      ...
      debugPrint('[Water] goal changed: $oldGoal → $adjusted, notice set');
      final prefs = await SharedPreferences.getInstance();
      await prefs.setInt('dailyGoal', adjusted);
      notifyListeners();
    } else if (announce) {
      _pendingGoalChangeNotice =
          'Klíma aktívna · cieľ $adjusted ml (klíma: $sign${adjustmentMl} ml)';
      debugPrint('[Water] goal unchanged but announcing: $adjusted');
      notifyListeners();
    } else {
      debugPrint('[Water] goal unchanged ($adjusted), no announcement');
    }
  }
