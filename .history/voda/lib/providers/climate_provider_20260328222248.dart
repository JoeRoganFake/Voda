...
class ClimateProvider extends ChangeNotifier {
  ...
  /// Called by ProxyProvider on every WaterProvider notify, including after
  /// its async _load() completes — so autoClimateAdjust is always up-to-date.
  void attachWaterProvider(WaterProvider wp) {
    _waterProvider = wp;
    // Cache may have loaded before this was wired — apply immediately if so.
    if (_status == ClimateStatus.loaded && _data != null) {
      wp.applyClimateAdjustment(_data!.waterAdjustmentMl);
    }
    // If auto-adjust is on, refresh on every app start (3-hour cache prevents
    // redundant network calls; expired cache = new fetch = new adjustment).
