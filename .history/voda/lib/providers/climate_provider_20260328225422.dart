...
class ClimateProvider extends ChangeNotifier {
  ...
  bool _autoClimateWasOn = false; // track previous toggle state
  String _lastAppliedDate = ''; // track which day we last applied an adjustment

  static String _today() => DateTime.now().toIso8601String().substring(0, 10);

  /// Inject WaterProvider so ClimateProvider can trigger auto-adjustment.
  /// Called by ProxyProvider on every WaterProvider notify — guard against
  /// re-applying on unrelated changes (e.g. the user drinking water).
  void attachWaterProvider(WaterProvider wp) {
    final isNewInstance = _waterProvider != wp;
    ...
    if (isNewInstance || autoJustTurnedOn || isNewDay) {
      ...
      // Force a fresh fetch on a new day so we get today's weather;
      // otherwise use cache (prevents redundant network calls).
