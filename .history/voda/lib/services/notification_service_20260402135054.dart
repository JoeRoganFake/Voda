...
class NotificationService {
  ...
  static Future<void> _midnightReschedule() async {
    ...
    // Note: This needs to read settings from SharedPreferences
    // For now, just log that it fired
    // The actual rescheduling will be handled when the app opens
  }
  ...
  static Future<DateTime?> scheduleForDay(
    ...
    if (Platform.isAndroid) {
      ...
      while (currentTime.day == now.day && currentTime.hour < endHour) {
        ...
        debugPrint('[NotificationService] 🔔 Alarm #$alarmId at ${currentTime.toString()}');
      ...
      debugPrint('[NotificationService] 🌙 Midnight reschedule set for ${midnight.toString()}');
      debugPrint('[NotificationService] ✅ Total scheduled: ${alarmId - 1} notifications');
    }
