...
class NotificationService {
  ...
  static Future<DateTime?> scheduleForDay(
    ...
    if (now.hour >= endHour || currentTime.hour >= endHour) {
      ...
      debugPrint(
        '[NotificationService] ⏰ Past active hours, scheduling for tomorrow at $startHour:00',
      );
    }
    ...
    if (Platform.isAndroid) {
      ...
      // Schedule midnight reschedule for the next day after scheduled notifications
      // If scheduling for tomorrow, midnight reschedule is for day after tomorrow
      final scheduledDay = firstNotification?.day ?? now.day;
      final midnight = DateTime(
        now.year,
        now.month,
        scheduledDay == now.day ? now.day + 1 : scheduledDay + 1,
        0,
        0,
      );
