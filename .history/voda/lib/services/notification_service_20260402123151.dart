...
class NotificationService {
  ...
  static Future<bool> requestPermissions() async {
    if (Platform.isAndroid) {
      ...
      final canExact = await plugin?.canScheduleExactAlarms();
      if (canExact == false) {
  ...
  static Future<void> scheduleReminders({
    ...
    // Use exact alarms when permitted so notifications fire on time.
    // Falls back to inexact if the user has revoked the exact-alarm permission.
    AndroidScheduleMode scheduleMode = AndroidScheduleMode.inexactAllowWhileIdle;
    if (Platform.isAndroid) {
      final plugin = _plugin.resolvePlatformSpecificImplementation<
          AndroidFlutterLocalNotificationsPlugin>();
      // canScheduleExactAlarms() returns null on pre-API-31 (permission not needed there).
      final canExact = await plugin?.canScheduleExactAlarms();
      if (canExact != false) {
