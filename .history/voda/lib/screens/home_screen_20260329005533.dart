...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    water.addListener(_onWaterProviderChanged);

    // Always trigger a fresh weather check on startup.
    // startupRefresh waits for WaterProvider prefs to load, then checks
    // autoClimateAdjust and fetches+applies+announces.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _showPendingNotice();
      context.read<ClimateProvider>().startupRefresh();
    });
  }

  @override
  void dispose() {
    context.read<WaterProvider>().removeListener(_onWaterProviderChanged);
    super.dispose();
  }
  ...
  void _showPendingNotice() {
    final water = context.read<WaterProvider>();
    final notice = water.pendingGoalChangeNotice;
    if (notice == null) return;
    water.clearGoalChangeNotice();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Row(
            children: [
              const Icon(Icons.thermostat_outlined, color: Color(0xFF38BDF8), size: 18),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              _QuickAddButton(
                label: '${water.cupSize}ml',
                subtitle: "Pohár",
                onTap: () => water.addWater(water.cupSize),
              ),
              const SizedBox(width: 12),
              _QuickAddButton(
                label: '${water.bottleSize}ml',
                subtitle: "Fľaša",
                onTap: () => water.addWater(water.bottleSize),
              ),
              const SizedBox(width: 12),
              _QuickAddButton(
                label: 'Vlastné',
