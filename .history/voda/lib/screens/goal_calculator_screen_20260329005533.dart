...
class _GoalCalculatorScreenState extends State<GoalCalculatorScreen> {
  ...
  void _tryRefreshClimate() {
    final climate = context.read<ClimateProvider>();
    // Re-fetch if not yet successfully loaded (error, permission denied, idle, loading-stuck)
    if (!climate.isLoaded) {
      climate.refresh(force: true);
    }
  }
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<ClimateProvider>().refresh(force: true);
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final result = baseResult != null ? (baseResult + climateAdj).clamp(500, 6000) : null;
      ...
      body: ListView(
        ...
        children: [
          ...
          Row(
            children: [
              _OptionChip(
                ...
                icon: Icons.male,
                selected: _gender == 0,
                onTap: () { setState(() => _gender = 0); _tryRefreshClimate(); },
              ),
              ...
              _OptionChip(
                ...
                icon: Icons.female,
                selected: _gender == 1,
                onTap: () { setState(() => _gender = 1); _tryRefreshClimate(); },
              ),
            ],
          ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _InputField(
                      ...
                      hint: '70',
                      suffix: 'kg',
                      onChanged: (_) { setState(() {}); _tryRefreshClimate(); },
                    ),
                  ],
                ),
              ),
          ...
          _ActivityCard(
            ...
            onTap: () { setState(() => _activity = 0); _tryRefreshClimate(); },
          ),
          ...
          _ActivityCard(
            ...
            onTap: () { setState(() => _activity = 1); _tryRefreshClimate(); },
          ),
          const SizedBox(height: 8),
          _ActivityCard(
            ...
            onTap: () { setState(() => _activity = 2); _tryRefreshClimate(); },
          ),
          const SizedBox(height: 8),
          _ActivityCard(
            ...
            onTap: () { setState(() => _activity = 3); _tryRefreshClimate(); },
          ),
          const SizedBox(height: 28),
...
class _ClimateInfoCard extends StatelessWidget {
  final ClimateProvider climate;
  const _ClimateInfoCard({required this.climate});
  ...
  IconData _icon(ClimateLevel level) => switch (level) {
  ...
  @override
  Widget build(BuildContext context) {
    // Loading or idle (not yet started)
    if (climate.status == ClimateStatus.idle ||
        (climate.status == ClimateStatus.loading && climate.data == null)) {
      return _shell(
    ...
    if ((climate.status == ClimateStatus.error ||
         climate.status == ClimateStatus.permissionDenied) &&
        climate.data == null) {
      return _shell(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(
                climate.status == ClimateStatus.permissionDenied
                    ? 'Prístup k polohe zamietnutý'
                    : 'Nepodarilo sa načítať počasie: ${climate.errorMessage ?? ''}',
                style: TextStyle(color: Colors.white.withOpacity(0.5), fontSize: 12),
              ),
            ),
            GestureDetector(
              onTap: () => context.read<ClimateProvider>().refresh(force: true),
              child: Icon(Icons.refresh, size: 16, color: Colors.white.withOpacity(0.3)),
            ),
          ],
        ),
      );
    ...
    return _shell(
      borderColor: level.color.withOpacity(0.35),
      child: Row(
        children: [
          Container(
            ...
            child: Icon(_icon(level), color: level.color, size: 20),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  _labelSk(level),
                  style: TextStyle(color: level.color, fontWeight: FontWeight.w700, fontSize: 14),
                ),
                Text(
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: adj == 0
                  ...
                  : (adj > 0 ? const Color(0xFF1C44B3) : const Color(0xFF065F46)).withOpacity(0.4),
              borderRadius: BorderRadius.circular(20),
            ),
            child: Text(
              adj == 0 ? '±0 ml' : '${adj > 0 ? '+' : ''}$adj ml',
              style: TextStyle(
                color: adj == 0
  ...
  Widget _shell({required Color borderColor, required Widget child}) {
    return Container(
