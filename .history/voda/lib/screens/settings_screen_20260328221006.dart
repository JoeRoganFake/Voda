      ...
      child: Row(
        children: [
          ...
          Switch(
            value: enabled,
            onChanged: (val) {
              context.read<WaterProvider>().setAutoClimateAdjust(val);
              if (val) {
                // Trigger a fresh fetch so adjustment applies immediately
                context.read<ClimateProvider>().refresh(force: true);
              } else {
                // Restore base goal
                final base = water.baseGoal;
                context.read<WaterProvider>().setDailyGoal(base);
              }
            },
