...
class _WaterPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (progress > 0) {
      ...
      // Create wave path — extend far beyond screen edges to cover corners on rotation
      final wavePath = Path();
      wavePath.moveTo(-size.width, size.height * 3);
      ...
      for (double x = -size.width; x <= size.width * 2; x += 4) {
        ...
        final wave2 = cos((normalizedX * 3 * pi) - wavePhase) * 8;
        final y = waterY + wave1 + wave2;
        wavePath.lineTo(x, y);
      }

      wavePath.lineTo(size.width * 2, size.height * 3);
      wavePath.close();
