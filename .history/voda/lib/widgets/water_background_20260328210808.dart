...
class _WaterBackgroundState extends State<WaterBackground>
  ...
  void initState() {
    ...
    _tiltAnim =
        Tween<double>(begin: _tiltAngle, end: _tiltAngle).animate(_tiltController);

    _accelerometerSubscription =
        accelerometerEvents.listen((AccelerometerEvent event) {
      // Low-pass filter to smooth the sensor data
      const double filterFactor = 0.1;
      _smoothedTiltAngle = _smoothedTiltAngle * (1 - filterFactor) +
          atan2(event.x, event.y) * filterFactor;

      // Animate the tilt angle change
      final newTilt = _smoothedTiltAngle;
      if ((newTilt - _tiltAngle).abs() > 0.02) {
        _tiltAnim = Tween<double>(begin: _tiltAngle, end: newTilt).animate(
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(, _tiltController]),
      builder: (_, __) {
        return CustomPaint(
          ...
          painter: _WaterPainter(
            ...
            tiltAngle: _tiltAnim.valuroller.value * 2 * pi,
